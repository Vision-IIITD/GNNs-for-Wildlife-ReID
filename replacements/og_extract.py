...
class OmniGlue:
  ...
  def FindMatches(self, image0: np.ndarray, image1: np.ndarray, kpts0: dict = None, kpts1: dict = None, match_threshold=0.0):
    """TODO(omniglue): docstring."""
    height0, width0 = image0.shape[:2]
    height1, width1 = image1.shape[:2]
#    import pdb; pdb.set_trace()
    if not kpts0 or not kpts1:
        sp_features0 = self.sp_extract(image0) # tuple of (keypoints, descriptors, scores)
        sp_features1 = self.sp_extract(image1)
    else:
        sp_features0 = (
            kpts0['keypoints'].squeeze(0),
            kpts0['descriptors'].squeeze(0),
            kpts0['keypoint_scores'].squeeze(0),
        )
        sp_features1 = (
            kpts1['keypoints'].squeeze(0),
            
            kpts1['descriptors'].squeeze(0),
            kpts1['keypoint_scores'].squeeze(0),
        )
    ...
    for i in range(match_indices.shape[0]):
      ...
      if (sp_features0[2][match[0]] > match_threshold) and (
          sp_features1[2][match[1]] > match_threshold
      ):
        keep.append(i)
